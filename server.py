...
#model = vosk.Model(r'C:\Users\Snehal Adbol\Downloads\vosk-model-en-us-0.22\vosk-model-en-us-0.22')
...
def voice():
    ...
    # Check if 'call_state' is defined in the session
    if 'call_state' not in session:
        session['call_state'] = 'start'  # Initialize call state

    call_state = session['call_state']

    if call_state == 'start':
        # Start the conversation with a menu
        gather = Gather(numDigits=1, action='/handle-key', method='POST')
        gather.say('to redirect your call, press 1. To end the call, press 2.')
        resp.append(gather)

    # Add more states and responses as needed
